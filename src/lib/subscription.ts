...
export const PLAN_CAPS: Record<Tier, {
  ...
  apiAccess: boolean;
  /** Tarif minimum affiché (€/mois). Business = "à partir de" car devis. */
  priceFromEur: number;
  /** Pricing par devis (Business) : pas de Stripe direct. */
  quoteOnly: boolean;
}> = {
...
export function lockedFeaturesFor(tier: Tier): Array<{ key: string; label: string; requires: Tier }> {
  ...
  if (!PLAN_CAPS[tier].smsEnabled) locked.push({ key: "sms", label: "Alertes SMS (sur devis)", requires: "business" });
  if (!PLAN_CAPS[tier].whatsappEnabled) locked.push({ key: "whatsapp", label: "Alertes WhatsApp (sur devis)", requires: "business" });
