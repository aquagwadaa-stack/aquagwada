  ...
  if (trend === "improving") {
    return (
      <span className="inline-flex items-center gap-1 rounded-full bg-success/10 border border-success/40 px-1.5 py-0.5 text-[10px] font-medium text-success">
        <TrendingDown className="h-3 w-3" /> en amÃ©lioration
      </span>
    );
  }
  if (trend === "worsening") {
...
function isOfficialSchedule(f: Forecast): boolean {
  return f.kind === "official_schedule" || f.basis?.startsWith("Planning officiel SMGEAG") === true;
}

function forecastLabel(f: Forecast): string {
  return isOfficialSchedule(f) ? "Tour d'eau programme" : "Risque estime";
}

function forecastDetail(f: Forecast): string {
  if (isOfficialSchedule(f)) return `Source officielle SMGEAG - confiance ${Math.round(f.confidence * 100)}%`;
  return `Prevision statistique - confiance ${Math.round(f.confidence * 100)}%`;
}

function forecastIntensity(f: Forecast): string {
  if (isOfficialSchedule(f)) return "bg-primary/10 border-primary/50";
  return f.probability >= 0.7 ? "bg-warning/25 border-warning/60" : "bg-warning/10 border-warning/40";
}

/**
 * Regroupe les coupures qui partagent exactement le mÃªme crÃ©neau (mÃªme start/end)
 * pour Ã©viter d'empiler 3 barres identiques quand seuls les secteurs changent.
 */
type GroupedOutage = {
  key: string;
  commune_id: string;
...
/**
 * Calcule un layout en "lanes" : les Ã©lÃ©ments qui se chevauchent sont
 * empilÃ©s verticalement au lieu de se superposer.
 * Renvoie pour chaque clÃ© l'index de lane et le nombre total de lanes.
 */
...
/** Affiche les coupures d'une journÃ©e sous forme de timeline horaire. */
export function DayTimeline({
  ...
  lockedCtaText = "Essai gratuit Pro 7 jours Â· sans engagement",
  ...
  emptyDescription = "Une ligne par commune, vue d'ensemble en un coup d'Å“il.",
  emptyShowCta = true,
}: {
  date: Date;
  outages: Outage[];
   ...
   * passÃ© un petit teaser aprÃ¨s "maintenant", avec un overlay CTA.
   * Les jours futurs sont entiÃ¨rement verrouillÃ©s.
   */
  lockedAfterNow?: boolean;
  lockedCtaText?: string;
  lockedCtaTo?: string;
  /** (DÃ©prÃ©ciÃ©) Fraction du reste du jour visible aprÃ¨s "maintenant" (0..1). IgnorÃ© si teaserHours est dÃ©fini. */
  teaserPercentOfRest?: number;
  /** Nombre d'heures de visibilitÃ© aprÃ¨s "maintenant" avant le verrouillage (par dÃ©faut 1h). */
  teaserHours?: number;
  /**
   * Mode multi-communes : si fourni, affiche une ligne horizontale par commune,
   * dans l'ordre, mÃªme si aucune coupure / prÃ©vision pour la journÃ©e.
    ...
    <div className="rounded-2xl border border-border bg-card p-4 sm:p-6 shadow-soft">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-muted-foreground">
          ...
          {dailyForecasts.length > 0 && ` Â· ${dailyForecasts.length} creneau${dailyForecasts.length > 1 ? "x" : ""} futur${dailyForecasts.length > 1 ? "s" : ""}`}
        </span>
      </div>
      ...
      <div className="relative">
        ...
        <div className="relative mt-2 space-y-2" style={multiMode ? { marginLeft: 96 } : undefined}>
          {/* pointillÃ©s horaires (toutes les heures) trÃ¨s discrets */}
          ...
          {!multiMode && outages.length === 0 && dailyForecasts.length === 0 && (
            <div className="rounded-lg border border-dashed border-border py-10 text-center text-sm text-muted-foreground">
              {isFuture ? "Aucun creneau futur pour cette date." : "Aucune coupure programmee ou detectee."}
            </div>
          )}

          {/* Mode classique : on rend toutes les coupures empilÃ©es */}
          {!multiMode && (() => {
                ...
                {layoutItems.map(({ key, g }) => {
                  ...
                  const sectorLabel = g.sectors.length === 0
                    ...
                    : g.sectors.length <= 2
                      ...
                      : `${g.sectors.length} secteurs concernÃ©s`;
                  return (
                    <div
                      key={key}
                    ...
                    >
                      <div className="flex items-center gap-2 text-[11px] font-medium truncate">
                        ...
                        <span>{formatHM(g.starts_at)}â€“{outageEndLabel(g.primary, end)}</span>
                        {g.primary.commune?.name && <span className="text-muted-foreground hidden sm:inline">Â· {g.primary.commune.name}</span>}
                      </div>
          ...
          {/* Forecasts (jaune, dashed) â€” mode classique */}
            ...
            return (
              <div className="relative" style={{ height: totalHeight }}>
                {items.map(({ key, f, seg }) => {
                  const lane = laneOf.get(key) ?? 0;
                  const top = lane * (laneHeight + 6);
                  const intensity = forecastIntensity(f);
                  const ForecastIcon = isOfficialSchedule(f) ? Clock : Sparkles;
                  return (
                    <div
                      key={key}
                      ...
                      title={f.basis ?? forecastLabel(f)}
                    >
                      <div className="flex items-center gap-2 text-[11px] font-medium truncate">
                        <ForecastIcon className="h-3 w-3 shrink-0" />
                        <span>{f.window_start?.slice(0, 5)}-{f.window_end?.slice(0, 5)} - {forecastLabel(f)}</span>
                        {f.commune?.name && <span className="text-muted-foreground hidden sm:inline">Â· {f.commune.name}</span>}
                      </div>
          ...
          {multiMode && communes!.map((c) => {
            ...
            // Compute lanes : on mÃ©lange coupures + prÃ©visions et on les empile.
            ...
            return (
              <div key={c.id} className="relative group" style={{ height: rowHeight }}>
                {/* Ã‰tiquette commune (en dehors du conteneur scaled grÃ¢ce au marginLeft parent) */}
                ...
                {isEmpty && (
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <span className="text-[10px] text-muted-foreground/70 italic">
                      {isFuture ? "Aucune prÃ©vision" : "Pas de coupure"}
                    </span>
                  </div>
                )}
                {cOutages.map((g) => {
                  ...
                  const sectorLabel = g.sectors.length === 0
                    ...
                    : g.sectors.length <= 2
                      ? ` Â· ${g.sectors.join(", ")}`
                      : ` Â· ${g.sectors.length} secteurs`;
                  return (
                    <div
                      key={g.key}
                      className={`absolute rounded border ${tone} px-1 overflow-hidden flex items-center`}
                      style={{ left: `${segment.left}%`, width: `${segment.width}%`, top, height: laneHeight }}
                      title={`${formatHM(g.starts_at)}â€“${outageEndLabel(g.primary, end)}${sectorLabel}`}
                    >
                      <span className="text-[10px] font-medium truncate">
                         {formatHM(g.starts_at)}â€“{outageEndLabel(g.primary, end)}{sectorLabel}
                      </span>
                    </div>
                  );
                })}
                {cForecasts.map((f) => {
                  ...
                  return (
                    <div
                      ...
                      style={{ left: `${segment.left}%`, width: `${segment.width}%`, top, height: laneHeight }}
                      title={`${forecastLabel(f)} - ${f.basis ?? ""}`}
                    >
                      <span className="text-[10px] font-medium truncate flex items-center gap-1">
                        <ForecastIcon className="h-2.5 w-2.5 shrink-0" />
                        <span className="truncate">{f.window_start?.slice(0, 5)}-{f.window_end?.slice(0, 5)}</span>
                        <span className="ml-auto pl-1 font-semibold text-warning-foreground/90">{isOfficialSchedule(f) ? "SMGEAG" : `${Math.round(f.probability * 100)}%`}</span>
                      </span>
                    </div>
                  );
                })}
          ...
          {/* Overlay verrouillage : voile floutÃ© + CTA */}
          {lockFromPct !== null && (
            <div
              ...
              <div className="relative w-full overflow-hidden rounded-md border border-dashed border-primary/30 bg-gradient-to-r from-background/60 via-background/85 to-background/95 backdrop-blur-[3px]">
                <div className="pointer-events-auto absolute inset-0 flex flex-col items-center justify-center gap-1 p-2 text-center">
                  ...
                  <p className="text-[11px] font-semibold leading-tight">
                    {isFuture ? "PrÃ©visions Pro" : "Reste de la journÃ©e"}
                  </p>
                  <Link
                    to={lockedCtaTo}
          ...
          {outages.map((o) => (
            <li key={o.id} className="py-3 flex flex-wrap items-center gap-3 text-sm">
              ...
              <span className="font-medium">
                {formatHM(o.starts_at)} â†’ {o.ends_at ? formatHM(o.ends_at) : <span className="text-muted-foreground italic">heure de retour estimÃ©e</span>}
              </span>
              <span className="text-xs text-muted-foreground">
                durÃ©e {formatDuration(durationBetween(o.starts_at, o.ends_at) ?? o.estimated_duration_minutes)}
              </span>
              {o.commune?.name && <span className="ml-auto text-xs text-muted-foreground">{o.commune.name}{o.sector ? ` Â· ${o.sector}` : ""}</span>}
            ...
            return (
              <li key={f.id} className="py-3 flex flex-wrap items-center gap-3 text-sm">
                <span className="inline-flex items-center gap-1 rounded-full border border-warning/50 bg-warning/10 px-2 py-0.5 text-[11px] font-medium text-warning-foreground">
                  <ForecastIcon className="h-3 w-3" /> {forecastLabel(f)}
                </span>
                <span className="font-medium">
                ...
                {!isOfficialSchedule(f) && <TrendBadge trend={f.trend} />}
...
/** SÃ©lecteur de jour : aujourd'hui Â± N. Style minimal, conforme au design. */
